...
* публикуем таблицу и обнуляем note и таб. со вседениями об доб. операции sRaportOperate
		...
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			...
			if(fromPHPobj["login"]==true){
				...
				if(fromPHPobj["status"]=="1"){
					...
					tblTrH+='<th id="thTime">Время (сек)</th>';
					...
					var per=data.month;
						if(fromPHPobj["data"][i]["r_id"]!=''){
							...
							tblTr += '</td><td class="tdCount">'+fromPHPobj["data"][i]["count"];
					...
					var tblTrKonto ='<table id="tblKonto" border="0"><tr id="trTable">';//	
							...
							tblTrKonto += '</td><td id="tdSumValue">'+fromPHPobj["konto"];
					...
					tblTr+='<th id="thTime">Время (сек)</th>';
...
xmlhttp.onreadystatechange=function()
  ...
  document.getElementById("selOrd").value=document.getElementById("sOrder").selectedIndex;
   ...
   if((data.a_id!==undefined) || (adata._id !="")){ 
					...
					if(operateList['login'] != false){
						...
						tblTr +='<tr class="trTable2"><td class="tdId">'+operateList['list']["serial"];//№	
								//	tblTr += '</td><td class="tdSection">'+operateList['list']["section"]+'</td>';
									...
									tblTr += '</td><td class="tdPrice">'+operateList['list']["price"];
									tblTr += '</td></tr>';
						tblTr+='</table>';
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
		...
		//alert(fromPHPobj["test"]);
			...
			alert("Что то не то с ответом :( btnAdd "+e);alert("Что то не то с ответом :( btnAdd "+e);
		} 

...
//alert("delSelModel.fromPHPs="+fromPHPs);
	try{
		...
		if(fromPHPobj.status==1){
			document.getElementById("note").innerHTML='<span id=\"spanNote\">Модель удалена</span>';
			loadSectionList();
		}
		if(fromPHPobj.status==0){
			document.getElementById("note").innerHTML='<span id=\"spanNote\">Модель не удалена</span>';
		}
...
//alert("delSelSection.fromPHPs="+fromPHPs);
	try{
		var fromPHPobj=!(/[^,:{}[]0-9.-+Eaeflnr-u nrt]/.test(fromPHPs.replace(/"(.|[^"])*"/g, ''))) && eval("(" + fromPHPs + ")");
		if(fromPHPobj.status==1){
			document.getElementById("note").innerHTML='<span id=\"spanNote\">Раздел удален</span>';
			loadSectionList();
		}
		if(fromPHPobj.status==0){
			document.getElementById("note").innerHTML='<span id=\"spanNote\">Раздел не удален</span>';
		}
...
//alert("btnAddClick.fromPHPs="+fromPHPs);
	...
	if(fromPHPobj.status==1){
		document.getElementById("note").innerHTML='<span id=\"spanNote\">Данные добавлены успешно</span>';
	}
	if(fromPHPobj.status==0){
		document.getElementById("note").innerHTML='<span id=\"spanNote\">Данные не добавлены</span>';
	}
...
//alert("f="+data.f);
	...
	if((data.descript!="" || data.time!="" || data.price!="") || (data.mName!='' && data.ml_id!=0) || (data.section!='' && data.sl_id!=0)){
...
//alert("btnUpdateClick.fromPHPs="+fromPHPs);
	...
	try{
		...
		if(fromPHPobj["status"]==1){			
			...
			selectSection();
			document.getElementById("note").innerHTML='<span id=\"spanNote\">Изменения выполнены</span>';
		}else{
			document.getElementById("note").innerHTML='<span id=\"spanNote\">Изменения не сделаны</span>';
		}
...
//alert("dataNewSection="+data.yesNewSection);
	...
	if( (data.descript=="" || data.time=="" || data.price=="") || (data.mName=="" && data.ml_id==0) || (data.nweSection=="" && data.sl_id==0)){
		alert("Вы не заполнили необходимые поля");
	}else{
...
//alert("fromPHPs="+fromPHPs);
		...
		if(fromPHPobj["status"]==true)document.getElementById("note").innerHTML='<span>Данные удалены!</span>';
		else document.getElementById("note").innerHTML='<span>Данные не удалены!</span>';

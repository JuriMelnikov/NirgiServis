...
//    xhr.open("POST","scr_worker.php?timeStamp="+new Date().getTime()+"&f=0",false);
//
//    xhr.send();
//    if(xhr.status !== 200){
//        alert(xhr.status+': '+xhr.statusText);
//    }else{
//        var fromPHPs = xhr.responseText;
//        if(!fromPHPs["loginCheck"]){
//            showLoginWindow();
...
function openAccountWindow(){
    ...
    if(!(data.w_id==="")){
        // Отображаем и центрируем окно
        var modalWindow = document.getElementById("account");
        modalWindow.css({
            position: 'absolute',
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("POST","fwSelectAccount",false);
        xhr.send("data="+JSON.stringify(data));
        if(xhr.status !== 200){
            alert(xhr.status+': '+xhr.statusText);
        }else{
            let collback = xhr.responseText;
            if(collback["login"]){
                document.getElementById("newLogin").value=collback["login"];
...
function btnAddClick(){
    document.getElementById("note").innerHTML="";
    let data=new Object();
    data.suname=document.getElementById("family").value; 
    data.name=document.getElementById("name").value;
    data.code=document.getElementById("kood").value;

    let listOrNew=document.getElementById("ListOrNew").value;
    let appointment={};
    if(listOrNew==="New"){data.appointment.name=document.getElementById("newAppointment").value;}
    if(listOrNew==="List"){data.appointment.name=document.getElementById("selectAppointment").value;}
    data.listOrNewAppointment=listOrNew;
    data.phone=document.getElementById("telefon").value;
    data.bankAccount=document.getElementById("bankCount").value;
    data.city=document.getElementById("city").value;
    data.address=document.getElementById("adress").value;
    let user={};
    data.user.login=document.getElementById("login").value;
    data.user.password=document.getElementById("password").value;
    ...
    if(data.family=="" || data.name =="" || data.kood=="" || data.appointment=="" || data.bankCount=="" || data.login=="" || data.password==""){
        alert("Вы не заполнили необходимые поля");
    }else{
        let xhr = new XMLHttpRequest();
        xhr.open("POST","fwAddClick",true);
        xhr.send("data="+JSON.stringify(data));
        if(xhr.status !== 200){
            alert(xhr.status+': '+xhr.statusText);
        }else{
            let collback= xhr.responseText;
            let newApp='';
            if(collback['newApp'])newApp='Добавлена новая должность';
            if(collback['status'])
